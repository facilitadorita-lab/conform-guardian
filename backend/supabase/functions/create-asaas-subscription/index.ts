...
Deno.serve(async (req: Request) => {
  ...
  if (!customerResponse.ok) return respond({ error: 'asaas_customer_error', details: customer }, 400)
  ...
  const { data: assinatura, error: assinaturaError } = await userClient.rpc('api_master_atualizar_assinatura', {
