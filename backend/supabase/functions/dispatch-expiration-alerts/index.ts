...
Deno.serve(async (req: Request) => {
  ...
  const { data: generated, error: generationError } = await supabase.rpc('gerar_alertas_vencimento')
  if (generationError) return json({ error: generationError.message }, 500)

  const { data: alerts, error: queueError } = await supabase
    .from('alertas')
    .select('id,titulo,mensagem,data_vencimento,usuarios!alertas_usuario_id_fkey(nome,email),empresas!inner(nome_fantasia)')
  ...
  for (const alert of alerts ?? []) {
    try {
      ...
      await supabase.from('alertas').update({ email_status: 'enviado', email_enviado_at: new Date().toISOString() }).eq('id', alert.id)
      sent++
    } catch (error) {
