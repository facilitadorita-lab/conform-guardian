...
Deno.serve(async (req: Request) => {
  ...
  if (!empresaId || !email || !nome || !allowedRoles.includes(perfil)) return respond({ error: 'invalid_payload' }, 400)

  const { data: canAdmin, error: permissionError } = await userClient.rpc('can_admin_company', { p_empresa_id: empresaId })
  if (permissionError || !canAdmin) return respond({ error: 'forbidden' }, 403)

  const redirectTo = Deno.env.get('INVITE_REDIRECT_URL')
  const { data: invited, error: inviteError } = await adminClient.auth.admin.inviteUserByEmail(email, { data: { nome }, redirectTo })
  if (inviteError || !invited.user) return respond({ error: inviteError?.message ?? 'invite_failed' }, 400)

  const { error: profileError } = await adminClient.from('usuarios').upsert({
    id: invited.user.id, nome, email, telefone: input.telefone ?? null, cargo: input.cargo ?? null, status: 'ativo', is_master: false,
  })
  ...
  const { error: membershipError } = await adminClient.from('usuarios_empresas').upsert({
    usuario_id: invited.user.id, empresa_id: empresaId, perfil, ativo: true, deleted_at: null,
